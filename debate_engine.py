...
def run_academic_summary(
    ...
    if all_papers:
        ...
        for i, p in enumerate(all_papers[:20], 1):
            ...
            if len(p.authors) > 3:
                authors_str += " et al."
            # v0.12.8: papers without abstracts may only be cited topically —
            # flag them so the summary prompt can steer the LLM away from
            # attributing concrete findings/methods to title-only papers.
            _abs_flag = "" if (getattr(p, "abstract", "") or "").strip() else " [NO-ABSTRACT]"
            ref_lines.append(f"[{i}] {authors_str} ({p.year}). {p.title}. {p.journal}, cited by {p.citation_count}. DOI: {p.doi or 'N/A'}{_abs_flag}")
        paper_references = "\n".join(ref_lines)
...
5. 在报告中适当引用真实论文的结论来支撑辩论观点，每篇参考文献至少在正文中被引用一次。引用格式必须使用数字方括号标记（如[1]、[2,3]、[1-3]），严禁使用作者-年份格式（如van Eck & Waltman (2017)）。引用编号必须与论点内容匹配——例如不要用一篇1997年的石油价格论文来支撑2020年代的可再生能源政策论点。每个引用必须对应与该论点主题相关的参考文献。正文中必须频繁出现[1]、[2]等引用标记——每个主要论点段落至少包含1个引用，不得只在末尾参考文献列表中列出编号而在正文中不引用。参考文献必须保持英文原文——严禁翻译任何文献标题、期刊名称、作者姓名（这是硬性要求，翻译后的标题等同于虚构文献）。另外：①纯背景常识陈述（如历史事件、学科共识，例如"20世纪70年代发生了石油危机"）不得挂任何引用标记——这类陈述无需文献支撑，强行挂引用会降低报告可信度；②参考文献列表中标记[NO-ABSTRACT]的文献表示系统未获取到其摘要，引用时仅可笼统提及研究主题（如"有研究考察了欧洲国家油价冲击的宏观效应[X]"），严禁归因具体数据、方法、模型或结论；③每处引用标记必须与该句论断直接相关——严禁将论断挂靠到主题无关的文献上（宁缺毋滥：句子没有合适文献支撑就不挂引用），且论断内容必须是所引文献摘要中明确陈述或可直接推论的内容，禁止基于文献主题自行演绎出摘要未覆盖的数据、机制、案例细节或结论
...
5. Cite real paper conclusions appropriately to support debate arguments, each reference should be cited at least once in the text. Citation format MUST use numeric bracket markers (e.g., [1], [2,3], [1-3]). Do NOT use author-year format (e.g., van Eck & Waltman (2017)). Citation numbers MUST match the claim content — do not use a 1997 oil price paper to support a 2020s renewable energy policy claim. Each citation must reference a paper topically relevant to the claim. In-text citations [1], [2] etc. MUST appear frequently throughout the body — every major argument paragraph must contain at least one citation marker. Do NOT list reference numbers only at the end without citing them in the body. References must remain in their original language — NEVER translate paper titles, journal names, or author names (translated titles are equivalent to fabricated references). Additionally: (a) pure background/common-knowledge statements (historical events, textbook consensus, e.g. "an oil crisis occurred in the 1970s") must NOT carry any citation marker — they need no literature support and forced citations hurt credibility; (b) references flagged [NO-ABSTRACT] in the list below have no abstract available — cite them by topical mention only (e.g., "one study examined oil price shocks in European economies [X]"), NEVER attribute specific data, methods, models, or conclusions to them; (c) every citation marker must be directly relevant to the claim it supports — NEVER hang a claim on a topically unrelated reference (if no reference fits, leave the sentence uncited), and each claim must stay within what the cited abstract explicitly states or directly implies — do NOT extrapolate data, mechanisms, case details, or conclusions beyond the abstract
