...
本报告对「{topic}」领域近5年学术文献进行系统性调研，检索并筛选{total}篇高质量论文（S级{s_count}篇、A级{a_count}篇）。核心发现：{method_keywords}是该领域当前主流方法，因果推断与深度学习融合成为新兴方向，但在多任务联合建模与跨领域迁移方面仍存在显著空白。"""
    ...
    def _build_final_findings(
        ...
        # 从共识结论提取
        if consensus_points:
            for i, point in enumerate(consensus_points[:3]):
                findings.append({
                    "text": point,
                    "confidence": "高",
                    "evidence": "",
    ...
    def _build_final_trends(self, papers: List[PaperCandidate]) -> str:
        ...
        if year_counts:
            ...
            lines.append(f"- **{years[0]}-{years[-1]}**：{len(papers)}篇论文覆盖该时期")
            if len(years) >= 3:
                lines.append(f"- **{years[-1]}年** 论文数量显著增长，反映领域关注度上升")
            lines.append(f"- **方法论转向**：从单一预测模型 → 因果推断+可解释性并重")
            lines.append(f"- **数据趋势**：多模态融合（文本+时序+图结构）成为新范式")
        else:
    ...
    def _build_final_gaps(
        ...
        gaps = [
            {
                "gap": "多任务联合建模",
                "detail": "当前研究多为单一预测任务，缺乏碳排放-电价-负荷的联合优化框架",
                "suggestion": "可尝试多任务学习（MTL）或多层次模型",
            },
            {
                "gap": "动态因果效应",
                "detail": "因果ML方法在能源政策评估中的应用仍处起步阶段",
                "suggestion": "结合DID设计+因果森林，评估碳市场政策的动态效应",
            },
            {
                "gap": "跨领域迁移学习",
                "detail": "不同能源市场（如EU-ETS vs 中国碳市场）间的模型迁移鲜有研究",
                "suggestion": "域自适应（Domain Adaptation）方法可降低新市场建模成本",
            },
            {
                "gap": "GNN在网络效应中的应用",
                "detail": "电力网络、碳市场关联网络的图结构信息未被充分利用",
                "suggestion": "图神经网络可建模区域间能源流动的溢出效应",
            },
            {
                "gap": "可解释性从选项到标配",
                "detail": "多数ML论文缺乏SHAP/LIME分析，政策制定者难以信任黑箱模型",
                "suggestion": "将可解释性分析作为论文标准输出",
            },
        ]

        for i, g in enumerate(gaps[:5], 1):
            lines.append(f"**{i}. {g['gap']}**")
            lines.append(f"- 现状：{g['detail']}")
