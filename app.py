...
_CV_EXPECTED_BUILD = "v0.12.18"  # bump together with VERIFIER_BUILD / marker
try:
    ...
    with st.expander("🔬 " + ("事实校验（自动阶段 · 一键全辩后执行）" if is_zh else "Fact Check (auto stage after full debate)"), expanded=False):
        ...
        if not _fc_report:
            ...
            if _fc_result and _fc_type == "citation":
                ...
                # v0.12.16: show only the REMAINDER outside the 📭/📖 tiers —
                # the raw total restated the same claims a second time
                _fc_unv_rest = max(0, int(_fc_result.unverified or 0)
                                   - int(getattr(_fc_result, "insufficient_evidence", 0) or 0)
                                   - int(getattr(_fc_result, "needs_fulltext", 0) or 0))
                c4.metric("❓ Unverified", _fc_unv_rest,
                          help=("未归入📭/📖的未验证论断（计入置信度分母）" if is_zh else
                                "Unverified claims outside the 📭/📖 tiers (counted in the score)"))
...
def main():
    ...
    st.caption("build: v0.12.18")  # 版本标记，确认部署用
    if CV_MODULE_STALE:
