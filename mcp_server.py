...
async def _handle_run_full_pipeline(params: dict) -> str:
    ...
    max_rounds = int(params.get("max_rounds", 8))
    skip_requirement = bool(params.get("skip_requirement", False))
    if not topic:
        return json.dumps({"status": "error", "error": "topic is required"}, ensure_ascii=False)
...
_register_tool(
    ...
    input_schema={
        ...
        "properties": {
            ...
            "skip_requirement": {"type": "boolean", "description": "Skip Phase 0-3 requirement research, reuse run_output/phase3_recommended_config.json (already generated by run_requirement_research)", "default": False},
        },
        "required": ["topic"],
    },
