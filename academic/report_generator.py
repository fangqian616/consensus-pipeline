...
24. 【v0.6.2引用硬约束】所有论文引用必须从论文清单中选取，禁止生成清单外的引用。每个结论必须标注支撑论文数量，格式：(N/M篇支撑，置信度🟢/🟡/🔴)，其中N=支撑论文数，M=该主题总论文数
25. 【v0.6.2置信度标注】每个核心结论后面必须标注置信度：(N/M篇支撑，置信度🟢高/🟡中/🔴低)，评判标准：N≥5且方法论一致→🟢；3≤N<5或存在争议→🟡；N<3→🔴
26. 【v0.6.2局限性必现】综述必须包含"检索边界与局限性"章节，说明：(a)检索源覆盖范围与盲区；(b)论文数量与代表性限制；(c)方法论评估的边界条件；(d)结论适用范围与外推风险"""
...
---结束---"""
    ...
    def _build_final_limitations(
        ...
        """v0.6.2: Search boundary and limitations section"""
        lines = ["## 检索边界与局限性", ""]
